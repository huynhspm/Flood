...
class FloodModule(LightningModule):
    ...
    def __init__(
        ...
        self.train_mae_hn = MeanAbsoluteError()
        self.val_mae_hn = MeanAbsoluteError()
        self.test_mae_hn = MeanAbsoluteError()
    ...
    def get_delta(self, input: torch.Tensor, cond: torch.Tensor):
        med_input, _ = input[:, :, :-2].median(dim=1, keepdim=True)
        med_cond, _ = cond[:, :, :-2].median(dim=1, keepdim=True)

        return med_input, med_cond
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor | None = None) -> torch.Tensor:
        ...
        med_input, med_cond = self.get_delta(x, cond)
        x[:, :, :-2] -= med_input
        cond[:, :, :-2] -= med_cond

        return self.net(x, cond) + med_input
    ...
    def training_step(
        ...
        self.train_mae_hn(preds[:, -1, 8], targets[:, -1, 8])
        self.log("train/loss", self.train_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/mae", self.train_mae, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/mae_hn", self.train_mae_hn, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        self.val_mae_hn(preds[:, -1, 8], targets[:, -1, 8])
        self.log("val/loss", self.val_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/mae", self.val_mae, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/mae_hn", self.val_mae_hn, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        self.test_mae_hn(preds[:, -1, 8], targets[:, -1, 8])
        self.log("test/loss", self.test_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("test/mae", self.test_mae, on_step=False, on_epoch=True, prog_bar=True)
        self.log("test/mae_hn", self.test_mae_hn, on_step=False, on_epoch=True, prog_bar=True)
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        cfg["net"]["pos_encoder"]["d_model"] = 32
        cfg["net"]["input_encoder"]["encoder_layer"]["d_model"] = 32
        cfg["net"]["cond_encoder"]["encoder_layer"]["d_model"] = 32
        ...
        loss, preds, _ = model.model_step((input, cond, output))
        print(loss)
        print(preds.shape)
